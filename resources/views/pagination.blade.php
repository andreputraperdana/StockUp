        ...
        <div class="list_notifikasi d-flex justify-content-center pt-4 pb-3"
            ...
            <div class="description_notif ps-5">
                ...
                @if (empty($Allbarang->Total))
                    ...
                    <input type="hidden" name="id_barang" class="id_barang_{{ $Allbarang->id }}"
                        value="{{ $Allbarang->id }}">
                    <input type="hidden" name="tipe_notif" class="tipe_notif_{{ $Allbarang->id }}"
                        value={{ !empty($Allbarang->Total) ? 'BarangHabis' : 'BarangKadaluarsa' }}>
                @elseif(!empty($Allbarang->Total))
                    <div class="desc_notif">
                        <p style="font-size: 16px;">Sisa Barang: {{ $Allbarang->Total }}</p>
                    </div>
                    <input type="hidden" name="id_barang" class="id_barang_{{ $Allbarang->barang_umkm_id }}"
                        value="{{ $Allbarang->barang_umkm_id }}">
                    <input type="hidden" name="tipe_notif" class="tipe_notif_{{ $Allbarang->barang_umkm_id }}"
                        value={{ !empty($Allbarang->Total) ? 'BarangHabis' : 'BarangKadaluarsa' }}>
                @endif
            </div>
            ...
            <div class="button__rekomen">
                <div class="button_rekomen_notif ps-5">
                    ...
                    <button type="submit" class="btn btn-primary " id="btn_rekom_notif_{{ $Allbarang->id }}"
                        value="{{ !empty($Allbarang->Total) ? $Allbarang->barang_umkm_id : $Allbarang->id }}"
                        onclick="onClickDetailBarang(this)"
